...
filter_access_to :collection => [:all]
  ...
  def company_social_roster
    ...
    @primary = @company.primaries.spouses
    @kid = (@company.soldiers.find(:all, :select => 'kids.*', :joins => [:kids]))
    @coffee = (@company.soldiers.find(:all, :order =>'primaries.lastname', :select => 'primaries.*', :joins => [:primaries], :conditions => ["rank IN (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) and primaries.relationship = ?", 'SFC','MSG','1SG','SGM','CSM', 'WO1','CW2','CW3','CW4','CW5','2LT','1LT','CPT','MAJ','LTC','COL', "Spouse"]))
