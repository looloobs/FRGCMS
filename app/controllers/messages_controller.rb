class MessagesController < ApplicationController
  ...
  def new
    ...
    if ["Battalion Commander","Command Sergeant Major","FRSA","Battalion FRG Leader","Battalion FRG Co-Leader","Rear-D Commander"].include?(@position)
      ...
      @bfrgleaders=(@user.battalion.users.find(:all, :conditions => ["position = ?",'FRG Leader'])+(@user.battalion.users.find(:all, :conditions => ["position = ?",'Battalion FRG Co-Leader']))+(@user.battalion.users.find(:all, :conditions => ["position = ?",'Battalion FRG Leader'])+(@user.battalion.users.find(:all, :conditions => ["position = ?",'FRSA'])))).collect(&:email).select{|s| !s.blank?}.join(", ")
      ...
      @spouses = ((Soldier.find_all_by_company_id(@company, :select => 'primaries.*', :joins => :primaries, :conditions => ['relationship = ? AND contacted = ?', 'Spouse', 'Yes']))+(@attached_platoon_primaries)).collect(&:email).select{|s| !s.blank?}.join(", ")
      ...
      @allcontacts=((@user.company.soldiers)+(Soldier.find_all_by_company_id(@company, :select => 'primaries.*', :joins => :primaries, :conditions => ['relationship = ? AND contacted = ?', 'Spouse', 'Yes']))+(@attached_platoon_primaries)+(Soldier.find_all_by_company_id(@company, :select => 'additionals.*', :joins => :additionals))).collect(&:email).select{|s| !s.blank?}.join(", ")
